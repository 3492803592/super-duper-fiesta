...
def remove_dangling_lines_by_polygon_conversion2(lines, tolerance=0.01):
    """
    使用线转面再面转线的方法去除悬挂线
    :param lines: LineString或MultiLineString几何对象
    :param tolerance: 坐标容差，用于处理浮点精度问题
    ...
    try:
        ...
        if not polygon.is_empty:
            ...
            # 用unary_union合并重叠线段，解决相邻面共享边被重复提取的问题
            boundary_lines = unary_union(boundary_lines)
...
def generate_parcels_with_fixed_params(input_shp, output_path_buffer, output_path_centerline=None,
    ...
    # 新增步骤：使用行政区划30公里缓冲区进行裁剪（如果提供了行政区划路径）
    # 使用缓冲区而非真实边界裁剪，避免省界附近路网被截断
    admin_boundary_geom_orig = None  # 保存原始坐标系下的行政边界，供后续精确裁剪使用
    if admin_boundary_path:
        try:
            ...
            # 保存原始行政边界几何（原始坐标系），供步骤10.5和11.5精确裁剪使用
            admin_boundary_geom_orig = admin_gdf.geometry.unary_union

            # 投影到米制坐标系创建30公里缓冲区
            admin_gdf_proj = admin_gdf.to_crs('EPSG:3857')
            admin_buffer_proj = admin_gdf_proj.geometry.unary_union.buffer(30000)
            admin_buffer_gdf = gpd.GeoDataFrame(geometry=[admin_buffer_proj], crs='EPSG:3857').to_crs(gdf.crs)

            # 用30公里缓冲区裁剪，保留边界附近完整路网
            gdf = gpd.clip(gdf, admin_buffer_gdf)
            print("已完成行政区划30公里缓冲区裁剪（保留完整路网，避免省界截断）")
    ...
    # --- 第一步：先对纯道路去悬挂线，清除OSM噪声和ArcPy毛刺 ---
    multi_line_roads_only = MultiLineString(line_strings)
    cleaned_roads_only = remove_dangling_lines_by_polygon_conversion(multi_line_roads_only, tolerance=0.01)

    if cleaned_roads_only is None or cleaned_roads_only.is_empty:
        print("警告: 纯道路去悬挂线后为空，使用原始线段继续!")
        cleaned_roads_only = multi_line_roads_only
    else:
        print("纯道路悬挂线清除完成")

    # 重新展平为列表供后续使用
    line_strings_cleaned = []
    if cleaned_roads_only.geom_type == 'LineString':
        line_strings_cleaned.append(cleaned_roads_only)
    elif cleaned_roads_only.geom_type == 'MultiLineString':
        line_strings_cleaned.extend(list(cleaned_roads_only.geoms))

    # --- 第二步：加入行政边界，封闭省界处截断的路 ---
    # 行政边界加入后，省界处截断的道路与边界围成封闭区域，保证地块完整
    admin_boundary_line = None
    line_strings_with_boundary = list(line_strings_cleaned)  # 基于清理后的纯道路列表

    if admin_boundary_path:
        try:
            admin_gdf_for_seal = gpd.read_file(admin_boundary_path).to_crs(target_crs)
            admin_boundary_geom_projected = admin_gdf_for_seal.geometry.unary_union

            # 提取行政边界的边界线（面 → 线）
            admin_boundary_line = admin_boundary_geom_projected.boundary
            if admin_boundary_line.is_empty:
                admin_boundary_line = None
            else:
                if admin_boundary_line.geom_type == 'LineString':
                    line_strings_with_boundary.append(admin_boundary_line)
                elif admin_boundary_line.geom_type == 'MultiLineString':
                    line_strings_with_boundary.extend(list(admin_boundary_line.geoms))
                print("已将行政边界加入线段集合用于封边")
        except Exception as e:
            print(f"加入行政边界封边失败: {e}，将继续使用纯道路线段")
            admin_boundary_line = None

    # 此时cleaned_line 含行政边界，用于步骤7缓冲区和步骤9 difference 生成地块
    cleaned_line = MultiLineString(line_strings_with_boundary)
    ...
    if admin_boundary_geom_orig is not None:
    # 将行政边界投影到目标坐标系
        admin_boundary_proj = gpd.GeoDataFrame(
            geometry=[admin_boundary_geom_orig], crs=original_crs
        ).to_crs(target_crs).geometry.values[0]
        study_area = admin_boundary_proj
        print("已使用行政边界作为研究区域边界")
    else:
    # 没有行政边界时才回退到凸包
        study_area = polygon.convex_hull
        print("已使用原始缓冲区凸包作为研究区域边界（无行政边界输入）")
    ...
    # 确保几何类型正确，将所有MultiPolygon拆散为独立Polygon
    if parcels.geom_type == 'Polygon':
        parcels = [parcels]
    elif parcels.geom_type == 'MultiPolygon':
        # 逐个展开，确保每个子面都独立参与面积过滤
        parcels = [poly for mp in parcels.geoms for poly in (list(mp.geoms) if mp.geom_type == 'MultiPolygon' else [mp])]
    else:
    ...
    # 步骤10.5：使用行政边界对地块进行精确裁剪
    if admin_boundary_geom_orig is not None:
        try:
            print("步骤10.5: 使用行政边界对地块进行精确裁剪...")
            admin_boundary_gdf_proj = gpd.GeoDataFrame(
                geometry=[admin_boundary_geom_orig], crs=original_crs
            ).to_crs(target_crs)
            parcel_gdf = gpd.clip(parcel_gdf, admin_boundary_gdf_proj)

            # 将裁剪后可能产生的MultiPolygon拆散为独立Polygon，再过滤碎片
            parcel_gdf = parcel_gdf.explode(index_parts=False).reset_index(drop=True)
            parcel_gdf = parcel_gdf[parcel_gdf.geometry.area >= 0].copy()
            print(f"行政边界精确裁剪完成，剩余 {len(parcel_gdf)} 个地块")
            if len(parcel_gdf) == 0:
                print("警告: 行政边界精确裁剪后地块为空!")
                return
        except Exception as e:
            print(f"地块精确裁剪失败: {e}，将保存未裁剪的地块")

    parcel_gdf.to_file(output_path_buffer, encoding='utf-8')
    print(f"已保存 {len(parcel_gdf)} 个地块要素到: {output_path_buffer}")

    # 步骤10.8：地块生成完毕，从线段集合中去掉行政边界，再去除悬挂线，生成输出用中心线
    print("步骤10.8: 去除行政边界线，对纯道路中心线去除悬挂线...")
    if admin_boundary_line is not None:
        try:
            # 用行政边界线对含边界的线段做difference，去掉边界线段
            boundary_buffer = admin_boundary_line.buffer(0.01)
            cleaned_line_no_boundary = cleaned_line.difference(boundary_buffer)
            print("已从中心线集合中去除行政边界线")
        except Exception as e:
            print(f"去除行政边界线失败: {e}，将使用含边界的线段继续")
            cleaned_line_no_boundary = cleaned_line
    else:
        cleaned_line_no_boundary = cleaned_line

    # 对去掉边界后的纯道路中心线去除悬挂线
    cleaned_line_for_output = remove_dangling_lines_by_polygon_conversion2(cleaned_line_no_boundary, tolerance=0.01)
    if cleaned_line_for_output is None or cleaned_line_for_output.is_empty:
        print("警告: 去除悬挂线后中心线为空，使用去边界前的线段!")
        cleaned_line_for_output = cleaned_line_no_boundary
    else:
        print("中心线悬挂线去除完成")
    ...
    if output_path_centerline:
        try:
            ...
            # 创建中心线的GeoDataFrame
            # 使用cleaned_line_for_output（纯道路中心线，不含行政边界）
            centerline_geoms_for_attr = []
            if cleaned_line_for_output.geom_type == 'MultiLineString':
                centerline_geoms_for_attr = list(cleaned_line_for_output.geoms)
            elif cleaned_line_for_output.geom_type == 'LineString':
                centerline_geoms_for_attr = [cleaned_line_for_output]
            ...
            # 步骤11.5：使用行政边界对中心线进行精确裁剪
            if admin_boundary_geom_orig is not None:
                try:
                    print("步骤11.5: 使用行政边界对中心线进行精确裁剪...")
                    admin_boundary_gdf_proj = gpd.GeoDataFrame(
                        geometry=[admin_boundary_geom_orig], crs=original_crs
                    ).to_crs(target_crs)
                    centerline_with_attrs = gpd.clip(centerline_with_attrs, admin_boundary_gdf_proj)
                    print(f"中心线精确裁剪完成，剩余 {len(centerline_with_attrs)} 条线段")
                except Exception as e:
                    print(f"中心线精确裁剪失败: {e}，将保存未裁剪的中心线")
...
if __name__ == "__main__":
    ...
    input_shp = r"./guangdong-260107-free.shp/gis_osm_roads_free_1.shp"#输入OSM路网数据
    admin_boundary_path = r"./行政边界/广州/广州.shp"  # 输入研究区域矢量数据
    output_buffer = r"./out/广州/guangzhou_parcel.shp"#输出社区地块面要素
    output_centerline = r"./out/广州/guangzhou_centerline.shp"#输出道路中心线
    output_dissolved_buffer_after_morph = r"./out/广州/guangzhou_dissolved_buffer.shp"
    output_precise_buffer = r"./out/广州/guangzhou_buffer.shp"#输出道路双线缓冲区
